...
import { chatbubbleOutline, heartOutline, heart, ellipsisVertical } from 'ionicons/icons';
...
const PostCard: React.FC<PostCardProps> = ({ post, showAuthor = true, routerLink, onOptions }) => {
  ...
  return (
    <IonCol size="12" sizeMd="6" sizeLg="6" sizeXl="4" key={post.id}>
      <IonCard className="post-card" routerLink={routerLink} button={!!routerLink}>
        {/* ─── صورة الغلاف (دائماً معروضة) ─── */}
        <IonImg src={coverImage} alt={post.title} className="post-card-image" />
        ...
        <IonCardContent>
          <IonGrid className="ion-no-padding">
                ...
                {showAuthor && post.User && (
                  <div className="post-card-author">
                    <IonAvatar className="post-card-avatar">
                      <IonImg src={getProfileImageUrl(post.User.ImageUrl)} alt={post.User.name} />
                    </IonAvatar>
                    <div className="post-card-author-info">
                      <IonText className="post-card-author-name">{post.User.name}</IonText>
            ...
            <IonCardSubtitle className="post-card-content">{post.content}</IonCardSubtitle>
          </IonGrid>
        </IonCardContent>
      </IonCard>
    </IonCol>
